...
import com.devpk.ApkManager.utils.*;
...
public class MainActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.appPreferences = MLManagerApplication.getAppPreferences(getApplicationContext());
        this.activity = this;
        this.context = this;

        new UtilsApp(context);
    ...
    private void setAppDir() {
        File appDir = UtilsApp.getAppFolder(context);
        if (!appDir.exists()) {
            appDir.mkdir();
        }
    }
    ...
    public static void setResultsMessage(Boolean results) {
        if (results) {
        ...
        switch (requestCode) {
            case MY_PERMISSIONS_REQUEST_WRITE_READ:
                if (grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                    UtilsDialog.showTitleContent(context, getResources().getString(R.string.dialog_permissions),
